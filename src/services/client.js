...
const KEYGEN_PUBLIC_KEY = ""; //remove this to publish to github
const KEYGEN_ACCOUNT_ID = ""; //same here;
...
function parseParameterizedHeader(header) {
  ...
  return keyvalues.reduce((o, [k, v]) => ((o[k] = v), o), {});
}
...
async function parse(req, res) {
  ...
  try {
    const { signature } = parseParameterizedHeader(
      res.headers.get("keygen-signature")
    );
      ...
      errors: [
        {
          ...
          detail: "Response signature was invalid",
...
export async function validateLicenseKeyWithFingerprint(key, fingerprint) {
  ...
  const { meta, data, errors } = await parse(req, res);
...
export async function activateMachineForLicense(
  license,
  fingerprint,
  name,
  platform,
  ...
  const req = new Request(
    `${KEYGEN_BASE_URL}/v1/accounts/${KEYGEN_ACCOUNT_ID ?? ""}/machines`,
    {
      method: "POST",
      headers: {
  ...
  const res = await fetch(req);
  const { data, errors } = await parse(req, res);
...
export async function deactivateMachineForLicense(license, id) {
  const req = new Request(
    `${KEYGEN_BASE_URL}/v1/accounts/${KEYGEN_ACCOUNT_ID ?? ""}/machines/${id}`,
    {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${license?.attributes?.metadata?.token ?? ""}`,
